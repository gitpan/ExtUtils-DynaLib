package ExtUtils::DynaLib::Struct;
require 5.002;
...
package ExtUtils::DynaLib::Struct::Imp;
...
sub Define {
    ...
    *{"${new_class}::TIESCALAR"} = \&ExtUtils::DynaLib::Struct::Imp::TIESCALAR;
    *{"${new_class}::AUTOLOAD"} = \&ExtUtils::DynaLib::Struct::Imp::AUTOLOAD;
    @{"${new_class}::ISA"} = qw (ExtUtils::DynaLib::Struct::Imp);
    my ($template, %fieldno) = ("");
	...
	ref ($fields) eq 'ARRAY'
	    or die 'Usage: Define ExtUtils::DynaLib::Struct( $struct_name, $template, \@field_names )';
	$template .= $template_fragment;
	my $i;
	for $i (0 .. $#$fields) {
	    defined (&{"ExtUtils::DynaLib::Struct::Imp::$fields->[$i]"})
